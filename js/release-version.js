...
function loadExperienceModules(){
  const files=['tool-panel-ux.js','canvas-style-pack.js','auto-dump-looks.js','project-batch.js','template-draft.js','project-previews.js','palette-workflow.js','appearance.js','home-compact.js','template-categories.js','new-design-carousel.js'];
  for(const file of files){
    const id=`washi-${file.replace(/\W+/g,'-')}`;
    if(document.getElementById(id))continue;
    const script=document.createElement('script');
    script.id=id;script.src=`./js/${file}?v=1.0&build=20260815-carouselfix`;script.async=false;
