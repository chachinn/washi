...
function upgradeCatalog(){
 ...
 for(const t of T.TEMPLATES){
   const comp=t.composition||t.layoutFamily||t.title||t.id;
   const key=`${t.category||''}|${t.format||''}|${comp}`;
   const n=counts.get(key)||0;
   if(n<7){kept.push(t);counts.set(key,n+1)}
 }
 ...
 function F(format,p){
   const d=T.FORMATS[format]||T.FORMATS.story,Wd=d.w,H=d.h;
   return {
    W:Wd,H,
    P:(x,y,w,h,r=0,rot=0,l='Add photo')=>photo(Wd*x,H*y,Wd*w,H*h,r,rot,l),
    B:(x,y,w,h,fill=p.paper,r=0,e={})=>shape(Wd*x,H*y,Wd*w,H*h,fill,r,e),
    X:(v,x,y,w,h,s=.04,c=p.ink,e={})=>text(v,Wd*x,H*y,Wd*w,H*h,Wd*s,c,e.font||'Georgia, serif',e),
    D:(v,x,y,s=.06,c=p.a,e={})=>sticker(v,Wd*x,H*y,Wd*s,{color:c,...e})
   }
 }
 ...
 function build(kind,format,p,v){
  ...
  switch(kind){
   ...
   case'type-cutout':a.push(f.B(0,0,.48,1,p.paper));a.push(f.P(.48,0,.52,1));a.push(f.X('L\nA\nT\nE\nL\nY',.1,.08,.28,.78,.085,p.ink,{bold:true,align:'center',lineHeight:.85}));a.push(f.D('⌁',.08,.78,.1,p.a));break;
 ...
 const fresh=[];
 fam.forEach(([kind,title,category],fi)=>{
  for(let v=0;v<6;v++){
   const format=formats[(fi+v)%formats.length],p=palettes[(fi*3+v)%palettes.length],objects=build(kind,format,p,v);
 ...
 T.TEMPLATES.splice(0,T.TEMPLATES.length,...first,...kept,...rest);
 ...
 T.LIBRARY_VERSION='3.2-curated-reference-pack';
}

function install(){
 const E=W.Editor,DB=W.DB,panel=document.querySelector('#panelContent');
 if(!E||!DB||!panel||panel.dataset.freedomReady)return;
 panel.dataset.freedomReady='2';let busy=false;
 const style=document.createElement('style');
 style.textContent=`
 .wt-color-studio{margin:14px 0;padding:14px;border:1px solid rgba(111,72,87,.13);border-radius:18px;background:rgba(255,255,255,.72)}
 .wt-color-head{display:flex;justify-content:space-between;gap:10px;align-items:end;margin-bottom:10px}.wt-color-head small{opacity:.62}
 .wt-color-main{display:grid;grid-template-columns:68px 1fr auto;gap:9px;align-items:center}.wt-color-main input[type=color]{width:68px;height:56px;padding:3px;border:1px solid rgba(80,50,60,.18);border-radius:14px;background:white}
 .wt-color-main input[type=text]{height:50px;border:1px solid rgba(80,50,60,.15);border-radius:14px;padding:0 12px;font:600 16px ui-monospace,SFMono-Regular,Menlo,monospace;text-transform:uppercase}
 .wt-color-main button,.wt-doc-color button{min-height:44px;border:1px solid rgba(80,50,60,.14);border-radius:13px;background:#fff;padding:0 12px;font-weight:700}
 .wt-doc-colors{display:grid;gap:8px;margin-top:12px}.wt-doc-color{display:grid;grid-template-columns:48px 1fr auto;gap:8px;align-items:center}.wt-doc-color input[type=color]{width:48px;height:44px;padding:2px;border:0;background:transparent}.wt-doc-color code{font-size:13px;opacity:.7}
 .wt-freedom{margin-top:12px}.wt-freedom-head{display:flex;justify-content:space-between;align-items:end;gap:8px}.wt-freedom-head small{opacity:.6}.wt-freedom-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:10px}.wt-freedom-grid label{display:grid;gap:5px;font-size:12px;font-weight:700}.wt-freedom-grid input,.wt-freedom-grid select{min-height:42px;border:1px solid rgba(80,50,60,.14);border-radius:12px;background:#fff;padding:6px}
 `;
