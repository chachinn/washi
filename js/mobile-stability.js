...
function stabilizeNav(){
 ...
 // iOS can transiently report an oversized safe-area inset while the visual
 // viewport is changing. The CSS caps the bar at 110px; clamp the measured
 // value too so editor offsets can never inherit a giant tab-bar height.
 const measured=Math.round(nav.getBoundingClientRect().height||76);
 const navHeight=Math.min(110,Math.max(76,measured));
...
W.MobileStability={sync,ensureServiceWorker,version:'v1.0-bottom-tabs-safe-cap'};
})();
